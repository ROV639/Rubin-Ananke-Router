...
- `luobin-ananke` 的“烧瓶小人观察者、七宗罪双面雷达、行为诊断不做人身攻击、冷水后必须给正面驱动”已补入 `Ananke_通用框架/07_人格`、`04_卡组`、`06_自检`、`09_部署`；“充分思考、防跳读、防自由发挥”已补入 `08_强约束`。前一版 Ananke 框架漏掉人格内核和推理链，这一点已修正，但仍需真实用例验证后才能转正。
...
- AI 工具层根目录扁平，只保留必要文件和一个全局提示词文件；`09_部署` 内按 Claude / ChatGPT / Grok 分成三个独立一键复制区。
- Agent 层保留完整 cards/bundles/agents，用于执行、验证和回写。
- `Ananke_通用框架` 必须显式承接 `luobin-ananke`，并保留对象澄清、证据分层、代价、反证、行动落点的推理链，否则只剩判断流程，没有人格承重，也无法约束模型自由发挥。
