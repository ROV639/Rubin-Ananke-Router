...
1. `Ananke_通用框架` 第一版漏掉 `luobin-ananke` 人格核心。
   ...
   - 已新增 `08_强约束_Ananke_充分思考与防自由发挥协议.md`。
   - 已同步更新 `00_总纲`、`01_必读`、`03_路由`、`04_卡组`、`05_模板`、`06_自检`、`09_部署`。
...
- `09_部署` 现为 Claude / ChatGPT / Grok 三个独立一键复制区，不再维护同内容的第二套复制块，避免双写漂移。

## 上架原则

- 首次创建时为 private；2026-06-29 按 Robin 要求改为 public。当前决定暂按公开仓库长期维护，方便其他 Agent/外部检查与后续复用；如后续纳入敏感配置或未脱敏材料，再单独评估是否改回 private。
...
3. `Ananke_通用框架` 修改时必须同时检查 `00/01/03/05/07/08/09`，防止推理链、人格层、强约束和全局提示词漂移。
