...
**Generate images and reference-image variants using your ChatGPT subscription — no `OPENAI_API_KEY` needed.**
...
That's it. Same wire protocol and same model quality. This uses ChatGPT subscription image generation, while still passing through the Codex backend gate; a `usage_limit_reached` response means that gate is closed, not that ChatGPT image generation or reference-image generation is unsupported.
...
| `--image`, `--reference PATH` | none | Local reference image for image-to-image generation. Repeat for multiple references. Supports `png`, `jpg`, `jpeg`, `webp`. |
...
| `-V`, `--version` | — | Print the CLI version (`chatgpt-imagegen 0.2.2`) and exit. |
...
# Image-to-image from a local reference
chatgpt-imagegen "create a warmer editorial variant while preserving the main subject" \
  --image ref.png -o variant.png --size 1536x1024
...
| Reference-image generation | ✅ via `--image` / `--reference` | Sends local images as `input_image` data URLs to the ChatGPT/Codex Responses backend. Good for variants, style transfer, and subject-preserving image-to-image generation. |
| Masked pixel edits (`/v1/images/edits`) | ❌ not exposed | This is not deterministic inpainting or mask editing. Use a dedicated editor/API when exact local edits are required. |
...
Robin's local agent workflow defaults to sequential generation. Do not run the same or substantially similar prompt multiple times. Parallel generation is allowed only after explicit confirmation, must use clearly different prompts, and is capped at 2 simultaneous processes.

Caveat: calls still pass through the Codex backend gate. If Codex/GPT-5.5 quota is closed, this CLI cannot start an image call even though the actual image capability comes from the ChatGPT subscription path. Don't run sustained batches; for bulk batches, use the official `/v1/images/generations` API with an `OPENAI_API_KEY`.
