...
Shared prompt guidance for both modes lives in `references/prompting.md` and `references/sample-prompts.md`. For Robin production work, `references/prompting.md` is mandatory pre-generation guidance, not optional inspiration.

## Project module routing

Before writing the final prompt, choose exactly one project module and note it in the Prompt Brief as `Project module selected`, `Not using`, and `Reason`. Do not blend project-specific templates into one global prompt.

Routing order: explicit user module > current project or directory match > content keyword match > project-local AGENTS/CANON > Robin global CANON > generic imagegen prompting.

Priority when multiple modules match: ROVxEVE > EVE > ONE_SPARK/UGC > project-local AGENTS/CANON > Robin global CANON > Generic.

Use these source modules only when matched:
- EVE / identity-lock / photoreal virtual portrait / mother-image work: read `/Users/robin/Rubin-Studio/✨Brainstorm - Agent🧵/04_参考与引用/EVE_通用框架/02_结构_EVE_提示词九段与冲突检测.md`; use the nine-part EVE structure with `CHARACTER LOCK` first and vary shot size, side, angle, lens, and composition across series.
- ROVxEVE / ROV + EVE / double-character anime tutorial / LifeOS / transparent UI work: read `/Users/robin/Rubin-Studio/✨Brainstorm - Agent🧵/04_参考与引用/ROVxEVE_通用框架/03_必读_ROVxEVE_画面UI与生图避坑.md` and `/Users/robin/Rubin-Studio/✨Brainstorm - Agent🧵/04_参考与引用/ROVxEVE_通用框架/06_模板_ROVxEVE_导演包硬字段必填版.md`; fill the lens-difference matrix, UI carrier, de-UI story, AI help point, UI alignment rule, scene, action, outfit, and expression.
- ONE SPARK / UGC / real-life Xiaohongshu snapshot work: read `/Users/robin/Rubin-Studio/✨Brainstorm - Agent🧵/04_参考与引用/ONE_SPARK_交友Hooks内容系统/M4_visual.md` and `/Users/robin/Rubin-Studio/✨Brainstorm - Agent🧵/04_参考与引用/ONE_SPARK_交友Hooks内容系统/M6_qc.md`; use Classic Chrome, low saturation, documentary realism, no front-facing face, one real-life moment, no text in image, no collage, no contact sheet. This is an anti-polish exception and must not inherit EVE cinematic portrait or Robin magazine-poster polish.
- Robin global CANON fallback: read `/Users/robin/Rubin-Studio/✨Brainstorm - Agent🧵/02_决定与共识/2026-06-11_审美CANON总纲_跨项目通用铁律_v1.md`; use retro restraint, Morandi low saturation, warm paper, negative space, magazine/print texture, and avoid fake text, watermarks, logos, plastic AI skin, HDR, and advertising gloss.
...
9. Build a modular Prompt Brief before any image generation or edit call. Do not pass the user's one-sentence request directly to `image_gen` or a CLI fallback.
10. The Prompt Brief must include, at minimum:
   - Use case / asset intent
   - Project module selected, Not using, Reason
   - Primary request
   - Reference image roles
   - Identity lock / style lock
   - Subject
   ...
   - Aspect ratio
11. For any series, batch, set, variants, or repeated assets, create a Variation Matrix before the first generation. Adjacent images must differ on at least 4 axes among shot size, camera height, lens/viewpoint, composition line, subject placement, action/pose, gaze, foreground/midground/background, lighting, UI carrier, scene, and emotional beat.
12. Augment the prompt based on specificity:
   - If the user's prompt is already specific and detailed, normalize it into a clear modular spec without adding creative requirements.
   - If the user's prompt is generic, add tasteful augmentation only when it materially improves output quality.
   - Never use a simple style-only prompt such as only subject + `cinematic`, `high detail`, `dramatic lighting`, or `shallow depth of field`.
13. Use the built-in `image_gen` tool by default.
14. For transparent-output requests, follow the transparent image guidance below: generate with built-in `image_gen` on a flat chroma-key background, copy the selected output into the workspace or `tmp/imagegen/`, run the installed `$CODEX_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py` helper, and validate the alpha result before using it. If this path looks unsuitable or fails, ask before switching to CLI `gpt-image-1.5`.
15. Inspect outputs and validate: subject, style, composition, text accuracy, and invariants/avoid items.
16. Iterate with a single targeted change, then re-check.
17. For preview-only work, render the image inline; the underlying file may remain at the default `$CODEX_HOME/generated_images/...` path.
18. For project-bound work, move or copy the selected artifact into the workspace and update any consuming code or references. Never leave a project-referenced asset only at the default `$CODEX_HOME/generated_images/...` path.
19. For batches or multi-asset requests, persist every requested deliverable final in the workspace unless the user explicitly asked to keep outputs preview-only. Discarded variants do not need to be kept unless requested.
20. If the user explicitly chooses or confirms the CLI fallback, then use the fallback-only docs for model, quality, size, `input_fidelity`, masks, output format, output paths, and network setup.
21. Always report the final saved path(s) for any workspace-bound asset(s), plus the project module selected, final prompt or prompt set, and whether the built-in tool or fallback CLI mode was used.
