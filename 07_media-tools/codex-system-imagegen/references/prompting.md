...
- [Anti-homogeneity requirement](#anti-homogeneity-requirement)
...
## Anti-homogeneity requirement

A prompt is not production-ready if it only names the subject and style. For Robin production work, build a modular Prompt Brief before generation; do not call an image tool with a one-sentence/simple prompt.

Every production prompt must make concrete choices for:
- shot size or crop
- camera angle / camera height
- lens, viewpoint, or perspective feel
- composition logic and subject placement
- lighting source and direction
- material, texture, or lived-in detail
- color / tonal language
- negative constraints

Avoid default AI-style filler as the main visual plan: `cinematic`, `ultra-detailed`, `dramatic lighting`, `high contrast`, `8k`, always-centered subject, always-shallow depth of field, glossy skin, plastic surfaces, HDR polish, and generic advertising gloss.

For any series, batch, set, variants, or repeated assets, create a Variation Matrix before the first generation. Adjacent images must differ on at least 4 axes among shot size, camera height, lens/viewpoint, composition line, subject placement, action/pose, gaze, foreground/midground/background, lighting, UI carrier, scene, and emotional beat.

Variation Matrix fields:

```text
Image | role | scene | shot size | camera height | lens/viewpoint | composition | subject placement | action/pose | gaze | foreground/midground/background | lighting | color temperature | anti-repeat note
```

Keep continuity at the identity, project, and style-lock level; vary the camera, composition, action, light, and visual rhythm. Do not create a set by reusing one visual skeleton with swapped props or backgrounds.
