...
当前按公开仓库维护，方便 Robin 让其他人和其他 Agent 检查、引用和持续改进。资料中包含本地路径、内部工作流和未转正的 agent 规则，外部使用者应按参考资料处理，不要直接当成可执行配置；如后续纳入敏感配置或未脱敏材料，再单独评估是否改回 private。
...
当前 `Ananke_通用框架` 还新增了 `08_强约束_Ananke_充分思考与防自由发挥协议.md`。它专门约束 AI 工具不要只按字面自由发挥，而要按“对象澄清 -> 事实分层 -> 代价计算 -> 偏差识别 -> 反证挑战 -> 行动落点”的链路思考。
